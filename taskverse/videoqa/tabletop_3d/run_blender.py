...
def get_exact_frame(current_keyframe_idx, total_num_frames):
	return (current_keyframe_idx) * (total_num_frames // 4) + 1
...
def rotate_and_keyframe(obj, degree, frame):
	bpy.ops.object.select_all(action='DESELECT')
	obj.select_set(True)
...
def move_to_xy_at_frame(obj, movement, frame):
	# Set the scene to the specific frame
	x, y = movement[0], movement[1]
	bpy.context.scene.frame_set(frame)
	new_location = Vector((x, y, 0))
	obj.location = obj.location + new_location
...
def run_render(metadata, save_image_path, use_cpu):
	...
	bpy.context.scene.frame_end = metadata['total_num_frames']
	...
	# set the first keyframe of video
	for obj in objs:
		rotate_and_keyframe(obj, 0, 1)
		move_to_xy_at_frame(obj, (0, 0), 1)

	# set other keyframes based on the metadata
	for idx, obj in enumerate(objs):
		for keyframe_order, keyframe_info in enumerate(metadata["keyframes"][idx]):
			if "rotation" in keyframe_info:
				rotate_and_keyframe(obj, keyframe_info["rotation"], get_exact_frame(keyframe_order, metadata['total_num_frames']))
			if "movement" in keyframe_info:
				move_to_xy_at_frame(obj, keyframe_info["movement"], get_exact_frame(keyframe_order, metadata['total_num_frames']))
	...
	setup_renderer(H=metadata["VIDEO_H"], W=metadata["VIDEO_W"], use_cpu=use_cpu)
	render_animation(save_image_path)
...
if __name__ == "__main__":
	...
	run_render(metadata, args.save_video_path, args.use_cpu)
